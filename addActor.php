...
function save_person_in_db( $data ) {
	...
	$find_sql          = $actor_find_sql;
	$alt_find_sql      = $director_find_sql;
	$person_insert_sql = $actor_insert_sql;
...
if ( isset( $_POST['form'] ) ) {
	...
	if ( empty( $form['first'] ) && empty( $form['last'] ) ) {
		$error_messages['name'] = '<strong><font color="red">Invalid:Either a first or last name <em>must</em> be specified.</font></strong><br>';
	} else {
		$default_values['first'] = (string)$form['first'];
		$default_values['last']  = (string)$form['last'];
	}
    /*
    if(empty($form_dob['day'])){
     	 $error_messages['dob'] = '<strong><font color="red">Invalid:Date of birth is empty.</font></strong><br>';
}
*/
	...
	$age_cutoff_date = strtotime( '1/1/1900' );
	...
	if ( ($dod_submitted && !$dod) || $dob < $age_cutoff_date || $dod > time() ) {
		...
		$dod = false;
	} else if ( $dob && $dod_submitted && $dod < $dob ) { // It is okay to submit dod without dob
...
?>
		<br>
		<h1>Add New Actor</h1>
		<!--<a href="<?php echo 'index.php';   ?>" style="float: right;">Back to Main Menu</a></li> -->
		<form action="<?php echo 'addActor.php'; ?>" method="POST">
			...
			Date of death: <?php echo build_date_form( 'form[dod]', $default_values['dod'] ); ?> (leave blank if alive now)
			<br>
			<input type="submit" value="Submit">
		</form>
        <a href="<?php echo 'index.php'; ?>" style="text-align:center">Back to Main Menu</a>
<?php page_footer(); ?>
